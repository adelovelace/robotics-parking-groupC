...
class BoundaryResult:
    ...
    empty_image_pixels: np.ndarray  # shape (M, 2), empty space pixels below boundary
    empty_robot_points: np.ndarray  # shape (M, 2), empty space points in the floor frame
...
class FloorProjectiveTransform(ProjectiveTransform):
    ...
    @staticmethod
    def extract_empty_pixels(
            contact_mask: np.ndarray,
            support_mask: np.ndarray,
            stride: int = 3,
            boundary_offset_px: int = 8
    ) -> np.ndarray:
        """
        Finds pixels vertically below the object boundary that belong to the support
        (floor) mask, effectively extracting empty space.
        Adds a safety boundary offset (5-10 pixels) to avoid marking empty space
        right up to the boundary walls.
        """
        empty_pixels = []
        y_idx, x_idx = np.where(contact_mask > 0)
        ...
        for x in unique_x[::stride]:
            # Identify the bottom-most boundary pixel in this column
            y_bottom = y_idx[x_idx == x].max()

            # Retrieve available support map pixels below the boundary, starting after the offset
            start_y = y_bottom + boundary_offset_px
            if start_y < support_mask.shape[0]:
                ys = np.where(support_mask[start_y:, x] > 0)[0] + start_y

                # Sub-sample Y for efficiency
                for y in ys[::stride]:
                    empty_pixels.append([x, y])

        if not empty_pixels:
            return np.empty((0, 2), dtype=np.int32)

        return np.array(empty_pixels, dtype=np.int32)
    ...
    def estimate_boundaries(
            ...
            *,
            ...
            empty_space_stride: int = 3,
    ) -> BoundaryResult:
        """
        Detect red object-floor boundaries and project them to the calibrated floor frame.
        Additionally estimates the empty space immediately preceding the boundary.
        """
        ...
        # Filter out non-finite coordinates to prevent projection NaNs/Infs
        if len(robot_points_raw) > 0:
            finite_mask = np.all(np.isfinite(robot_points_raw), axis=1)
            robot_points_raw = robot_points_raw[finite_mask]
            image_pixels = image_pixels[finite_mask]
            image_component_labels = image_component_labels[finite_mask]
        ...
        # Determine the empty space footprint extending outwards from boundary (adding safety offset of 8px)
        empty_image_pixels = self.extract_empty_pixels(
            contact_mask, support_mask, stride=empty_space_stride, boundary_offset_px=8
            )
        empty_robot_points_raw = self.image_to_floor(empty_image_pixels)

        # Filter non-finite empty space coordinates to maintain stability
        if len(empty_robot_points_raw) > 0:
            finite_mask = np.all(np.isfinite(empty_robot_points_raw), axis=1)
            empty_robot_points = empty_robot_points_raw[finite_mask]
            empty_image_pixels = empty_image_pixels[finite_mask]
        else:
            empty_robot_points = empty_robot_points_raw
        ...
        return BoundaryResult(
            ...
            empty_image_pixels=empty_image_pixels,
            empty_robot_points=empty_robot_points
        )
    ...
    def estimate_boundaries_world(
            ...
            **kwargs,
    ) -> tuple[np.ndarray, np.ndarray, BoundaryResult]:
        """
        Estimate boundary points and convert them from robot frame to global world frame.
        Returns:
            - world_points (boundary)
            - empty_world_points (empty spaces)
            - Result object
        """
        result = self.estimate_boundaries(image, **kwargs)
        world_points = self.robot_to_world(result.robot_points, robot_pose)
        empty_world_points = self.robot_to_world(result.empty_robot_points, robot_pose)
        return world_points, empty_world_points, result
